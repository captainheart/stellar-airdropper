...
const API_KEY = 'c8adad2534a24e3225f9cd8cfe527e26';
...
export default {
  getRatio: function (key, cb, cbErr) {
    if(key === 'xlm_to_usd' || key === 'xlm_to_gbp' || key === 'xlm_to_euro'){
      var se = '';
      if(key === '') se = 'USD';
      if(key === '') se = 'GBP';
      if(key === '') se = 'EUR';
      ...
      fixer.latest({ access_key: API_KEY })
      .then((data) => {
        if(data.success){
          var rlt = 0;
          if(key === 'usd_to_euro') {
            rlt = data.rates.USD
          } else if(key === 'gbp_to_euro') {
            rlt = data.rates.GBP
          } else if(key === 'usd_to_gbp') {
            rlt = data.rates.USD / data.rates.GBP
          }
          cb(rlt);
        } else{
          cbErr('failed');
