          ...
          allPairs.forEach((pair) => {
            ...
            Api.findOrder(this.$store.getters.offers, pair, (orders) => {
              ...
              if (orders.bidOrders.length === 0) {
                    ...
                    if (bidPrice !== null) {
                      ...
                      if (amount === 0) {
                        ...
                        Api.makeOrder(
                          ...
                          (res) => {
                            Api.getRatio(pair.ratiooption, (ress) => {
                              alert('buy');
                              alert(pair.ratiooption);
                              alert(ress);
                              ...
                              this.$store.commit('updateExchangePairOrder', data);
                            }, (errr) => {
                              window.Sconsole(['getting_ratio_from_api', errr], 'msg');
                            });
                ...
                // Check the real ratio of our real world every 10 min.
                const unix = Math.round(+new Date() / 1000);
                if (Math.abs(unix - pair.buy_time) >= 60 * 1000) {
                  Api.getRatio(pair.ratiooption, (ress) => {
                    if (Math.abs(ress - pair.buy_ratio) > 0.01) {
                      // current order
                      const currentOrder = orders.bidOrders[0];
                      // cancel current order
                      Api.cancelOrder(
              ...
              if (orders.askOrders.length === 0) {
                ...
                // Check the real ratio of our real world every 10 min.
                const unix = Math.round(+new Date() / 1000);
                if (Math.abs(unix - pair.sell_time) >= 60 * 1000) {
                  Api.getRatio(pair.ratiooption, (ress) => {
                    if (Math.abs(ress - pair.sell_ratio) > 0.01) {
                      // current order
                      const currentOrder = orders.askOrders[0];
                      // cancel current order
                      Api.cancelOrder(
                        ...
                        currentOrder,
                        (transResult) => {
                          window.Sconsole(['cancel sell base order result', transResult], 'msg');
                        }, (errRes) => {
                          window.Sconsole(['cancel sell base order err', errRes, errRes.message], 'msg');
                        });
                    }
                  }, (errr) => {
                    window.Sconsole(['getting_ratio_from_api', errr], 'msg');
